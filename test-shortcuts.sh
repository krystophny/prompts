...
for script in plan work play; do
    if [ -f "bin/$script" ] && [ -x "bin/$script" ]; then
        echo "✅ PASS: bin/$script exists and is executable"
        ((TEST_PASSED++))
    else
        echo "❌ FAIL: bin/$script missing or not executable"
        ((TEST_FAILED++))
    fi
done
echo ""
...
# Test plan script
run_test "plan script execution" "./bin/plan" "QADS Planning Workflow"
run_test "plan mentions chris-architect" "./bin/plan" "chris-architect"
run_test "plan mentions BACKLOG.md" "./bin/plan" "BACKLOG.md"

# Test work script  
run_test "work script execution" "./bin/work" "QADS Work Workflow"
...
# Test play script
run_test "play script execution" "./bin/play" "QADS Play Workflow"
...
if grep -q "chris-architect" bin/plan && grep -q "max.*sergei.*patrick" bin/work && grep -q "defects" bin/play; then
    echo "✅ PASS: Shortcuts match documented agent workflows"
    ((TEST_PASSED++))
else
    echo "❌ FAIL: Shortcuts don't match documented workflows"
    ((TEST_FAILED++))
fi
...
if [ $TEST_FAILED -eq 0 ]; then
    echo "🎉 All tests passed! Workflow shortcuts are working correctly."
    exit 0
else
    echo "💥 Some tests failed. Please review and fix issues above."
    exit 1
fi